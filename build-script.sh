...
BUILD_RESPONSE=$(curl -s -X POST https://cloudbuild.googleapis.com/v1/projects/safes-project/builds?alt=json \
  ...
  -d '{
    ...
    if [[ $BUILD_RESPONSE == *"\"status\": \"ERROR\""* ]]; then
    ...
    echo "Error: No container artifact URL found. Check Cloud Build logs for details."
    exit 1
    fi    
